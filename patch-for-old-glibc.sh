...
# Source File https://gist.github.com/dakkshesh07/240736992abf0ea6f0ee1d8acb57a400
...
curl -L https://archlinux.org/packages/extra/x86_64/patchelf/download | tar -C patchelf-temp --zstd -xf -
mv "$HOME"/patchelf-temp/usr/bin/patchelf "$HOME"/
rm -rf "$HOME"/patchelf-temp
echo "Downloading latest glibc from ArchLinux repos"
mkdir -p glibc
curl -L https://archlinux.org/packages/core/x86_64/glibc/download | tar -C glibc --zstd -xf -
curl -L https://archlinux.org/packages/core/x86_64/lib32-glibc/download | tar -C glibc --zstd -xf -
